...
import { QuestionType } from "./QuestionEditor";
        ...
        <FormControl id={`options[${index}]`} isInvalid={Boolean(touched && error)}>
            <Stack direction="row">
                <Input
                    ...
                    onChange={(event) => {
                        form.setFieldValue(`options[${index}]`, event.target.value).catch(error => {
                            console.log(error)
                        })
                        form.setTouched({
                            ...form.touched,
                            options: { ...form.touched.options, [index]: true }
                        }).catch(error => {
                            console.log(error)
                        });
                    }}
                        ...
                        <VStack alignItems="start" spacing="1">
                            {options.map((option, index) => (
                                <OptionField
                                    ...
                                    index={index}
                                    option={option}
                                    touched={touchedOptions[index] ?? false}
